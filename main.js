...
import { initFirebase, firebaseReady, signUpWithEmail, signInWithEmail, signOutUser, onAuthStateChangedHandler } from './firebase.js';
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	if (!statusBar) {
		...
		statusBar.style.cssText = 'position:absolute;top:.5rem;right:.75rem;font-size:.65rem;font-weight:600;letter-spacing:.5px;padding:.4rem .65rem;border-radius:12px;background:rgba(255,255,255,.15);backdrop-filter:blur(6px);color:#eaffea;display:flex;align-items:center;gap:.5rem;';
		authCard?.appendChild(statusBar);
	}

	function setStatus(msg, type='info') {
		if (!statusBar) return;
		const colors = { info: 'rgba(255,255,255,.15)', success: 'rgba(110,216,107,.25)', error: 'rgba(255,90,90,.3)' };
	...
	} catch (e) { setStatus('Firebase init failed', 'error'); console.error(e); }
	...
	function setMode(mode) {
		tabs.forEach(t => {
			...
			const panelId = active ? (mode === 'signin' ? 'signinForm' : 'signupForm') : null;
	...
	switchButtons.forEach(btn => btn.addEventListener('click', () => setMode(btn.dataset.switch)));
	...
	signinForm?.addEventListener('submit', async e => {
		...
		let ok = true;
		if (!email.value || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.value)) { setError('signinEmail', 'Enter a valid email'); ok = false; }
		if (!pass.value || pass.value.length < 6) { setError('signinPassword', 'Password too short'); ok = false; }
		if (!ok) return;
	...
	signupForm?.addEventListener('submit', async e => {
		...
		let ok = true;
		if (!first.value.trim()) { setError('firstName', 'Required'); ok = false; }
		if (!email.value || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.value)) { setError('signupEmail', 'Invalid email'); ok = false; }
		if (!pass.value || pass.value.length < 6) { setError('signupPassword', 'Min 6 chars'); ok = false; }
		if (pass.value !== confirm.value) { setError('confirmPassword', 'Passwords do not match'); ok = false; }
		if (!terms.checked) { setError('terms', 'You must accept terms'); ok = false; }
		if (!ok) return;
		setStatus('Creating account...');
		try {
			await signUpWithEmail(email.value, pass.value, { displayName: first.value.trim() });
	...
	document.querySelector('.tabs')?.addEventListener('keydown', e => {
		...
		else if (e.key === 'ArrowLeft') nextIndex = (currentIndex - 1 + order.length) % order.length;
		...
		if (user) {
			...
			if (!btn) {
				...
				btn.addEventListener('click', async () => {
					...
					try { await signOutUser(); } catch(e){ console.error(e); setStatus('Sign out failed','error'); }
				});
			}
